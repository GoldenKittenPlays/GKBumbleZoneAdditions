...
public class ModEvents
{
    ...
    public void playerTradedEvent(ScreenEvent.MouseButtonPressed event)
    {
        ...
        if (Screen.hasShiftDown())
        {
            ...
            if (item1.is(ModItems.BUMBLE_TRADER_TREAT.get()) || item2.is(ModItems.BUMBLE_TRADER_TREAT.get()))
            {
    ...
    public void entityRightClicked(PlayerInteractEvent.EntityInteract event)
    {
        ...
        if (!player.level().isClientSide())
        {
            if (event.getTarget() instanceof TraderBeeEntity bee)
            {
                ...
                if (itemstack.getItem() != ModItems.BUMBLE_TRADER_SPAWN_EGG && bee.isAlive() && !bee.isTrading() && !bee.isSleeping() && !player.isSecondaryUseActive())
                {
                    if (bee.isBaby())
                    {
                        //this.setUnhappy();
